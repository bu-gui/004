...
{
    uint8_t id = 0;
    mpu6050_read_regs(WHO_AM_I, &id, 1);
    ...
    mpu6050_write_reg(SMPLRT_DIV, 19);
    mpu6050_write_reg(CONFIG, 0x01);
    mpu6050_write_reg(GYRO_CONFIG, 0x00);
    mpu6050_write_reg(ACCEL_CONFIG, 0x00);
    return ESP_OK;
}

esp_err_t mpu6050_read_all(float *accel_x, float *accel_y, float *accel_z,
                           float *temp, float *gyro_x, float *gyro_y, float *gyro_z)
{
    uint8_t buf[14];
    ...
    *accel_x = (float)ax / ACCEL_SCALE - accel_offset_x;
    *accel_y = (float)ay / ACCEL_SCALE - accel_offset_y;
    *accel_z = (float)az / ACCEL_SCALE - accel_offset_z;
...
void mpu6050_calibrate(void)
{
    ...
    for (int i = 0; i < 100; i++) {
        mpu6050_read_all(&ax, &ay, &az, &t, &gx, &gy, &gz);
        sum_x += ax;
        sum_y += ay;
        sum_z += az;
    ...
    accel_offset_x = sum_x / 100.0f;
    accel_offset_y = sum_y / 100.0f;
    accel_offset_z = sum_z / 100.0f;
